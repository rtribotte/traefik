// Package pluginA is a Traefik local (Yaegi) middleware plugin used by the
...
// PluginA delays each request by a random duration before passing it on.
type PluginA struct {
	next http.Handler
	name string
}
...
func New(_ context.Context, next http.Handler, _ *Config, name string) (http.Handler, error) {
	return &PluginA{next: next, name: name}, nil
}

func (p *PluginA) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
