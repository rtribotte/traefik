...
Starting with `v3.8.0`, the Kubernetes Gateway API provider generates a multi-layer router structure for TLS-terminated (HTTPS) listeners.

Each such listener now produces a **parent router**, scoped to the listener server name with a `Host` matcher and holding the listener TLS configuration.
