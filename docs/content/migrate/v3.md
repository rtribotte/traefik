...
Each such listener now produces a **parent router**, scoped to the listener server name with a `Host` matcher, holding the listener entry point and, for a TLS-terminated (HTTPS) listener, its TLS configuration.
